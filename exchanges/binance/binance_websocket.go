...
const (
	binanceWebsocketURL      = "wss://stream.binance.com:9443"
	binanceWebsocketBase     = "/ws/"
	binanceWebsocketAggTrade = "@aggTrade"
	binanceWebsocketCandles  = "@kline_"
	binanceWebsocketTicker   = "@ticker"
	binanceWebsocketDepth    = "@depth"
)

// WebsocketSubscribe subscribes to a websocket connection
func (b *Binance) WebsocketSubscribe(product string, conn *websocket.Conn) error {
...
func (b *Binance) WebsocketClient() {
	for b.Enabled && b.Websocket {
		...
		conn, _, err := Dialer.Dial(binanceWebsocketURL, http.Header{})
		...
		for _, x := range currencies {
			err = b.WebsocketSubscribe(x, conn)
			if err != nil {
		...
		if b.Verbose {
			log.Printf("%s Subscribed to product messages.", b.GetName())
		}
		...
		conn.Close()
		log.Printf("%s Websocket client disconnected.", b.GetName())
	}
}
